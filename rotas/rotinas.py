...
# --- SOLUÇÃO PROBLEMA A: Função para pegar o cliente apenas quando necessário ---
def get_google_client():
    api_key = os.getenv("GENAI_API_KEY")
    if not api_key:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail="Chave de API do Google (GENAI_API_KEY) não configurada."
        )
...
def gerar_agenda(
    ...
    # Chama a função segura para obter o cliente
...
# --- SOLUÇÃO PROBLEMA B: Rota Limpa usando verificar_token ---
@rotinas_router.get("/listar", response_model=list[RotinaResponse])
async def listar_rotinas_usuario_logado(
    usuario: Usuario = Depends(verificar_token), # Isso substitui todo o código manual de JWT
    session: Session = Depends(pegar_sessao)
):
    """Lista todas as rotinas criadas pelo usuário autenticado"""
    
    # O 'usuario' já chega aqui validado e carregado do banco
    rotinas = session.query(Rotina).filter(Rotina.id_usuario == usuario.id).all()
    
    if not rotinas:
        # Nota: Retornar lista vazia [] costuma ser melhor que erro 404 em listas, mas mantive sua lógica
...
async def marcar_rotina_concluida(
    ...
    # Garante que o usuário só altere sua própria rotina
    if rotina.id_usuario != usuario.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Sem permissão para alterar esta rotina.")

    rotina.concluido = True # Certifique-se que o modelo Rotina tem essa coluna no models.py
    session.commit()
